# GCS Spark Data Product

A layered Vulcan data product on **Spark + Iceberg** (`gcslhdepot`, a GCS
...
```
quick-sanity/
├── config.yaml                 # Vulcan project config (Spark gateway -> gcslhdepot depot)
...
**Staging** (`gcslhdepot.staging_v6_vnew`, `kind SEED`) — one seed model per CSV,
...
**Intermediate** (`gcslhdepot.intermediate_v6_vnew`, `kind FULL`):
...
**Mart** (`gcslhdepot.mart_v6_vnew`, `kind FULL`) — consumption-ready:
...
```bash
vulcan fetchdf "select * from gcslhdepot.mart_v6_vnew.mart_sales_summary order by total_net_amount desc limit 10"
```
