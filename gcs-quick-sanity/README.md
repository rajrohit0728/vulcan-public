...
**Staging** (`gcslhdepot.staging_v5_vnew`, `kind SEED`) — one seed model per CSV,
...
**Intermediate** (`gcslhdepot.intermediate_v5_vnew`, `kind FULL`):
...
**Mart** (`gcslhdepot.mart_v5_vnew`, `kind FULL`) — consumption-ready:
...
```bash
vulcan fetchdf "select * from gcslhdepot.mart_v5_vnew.mart_sales_summary order by total_net_amount desc limit 10"
```
