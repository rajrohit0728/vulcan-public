MODEL (
  ...
  description 'Order-line grain fact table. Excludes cancelled orders and adds a TIMESTAMP(6) copy of order_date (order_timestamp) for time-series metrics, alongside the original DATE column used by DQ/summary grouping.',
  columns (
...
SELECT
  ...
  order_id,
  order_date,
  CAST(order_date AS TIMESTAMP(6)) AS order_timestamp,
  order_status,
  channel,
  customer_id,
  region,
