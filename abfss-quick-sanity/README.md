...
**Staging** (`abfsslhdepotrr.staging`, `kind SEED`) — one seed model per CSV,
...
**Intermediate** (`abfsslhdepotrr.intermediate`, `kind FULL`):
...
**Mart** (`abfsslhdepotrr.mart`, `kind FULL`) — consumption-ready:
...
```bash
vulcan fetchdf "select * from abfsslhdepotrr.mart_v3.mart_sales_summary order by total_net_amount_usd desc limit 10"
```
