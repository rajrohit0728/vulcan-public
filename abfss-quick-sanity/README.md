...
**Staging** (`abfsslhdepotrr.staging_v6_vnew`, `kind SEED`) — one seed model per CSV,
...
**Intermediate** (`abfsslhdepotrr.intermediate_v6_vnew`, `kind FULL`):
...
**Mart** (`abfsslhdepotrr.mart_v6_vnew`, `kind FULL`) — consumption-ready:
...
```bash
vulcan fetchdf "select * from abfsslhdepotrr.mart_v6_vnew.mart_sales_summary order by total_net_amount_usd desc limit 10"
```
